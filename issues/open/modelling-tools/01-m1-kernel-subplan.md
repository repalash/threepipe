...
| `../ops/bevel.ts` + 10 modules | `bmo_bevel.cc` + `bmesh_bevel.cc` (8.5k lines). Every option except custom profiles (`CurveProfile` is a separate subsystem), harden-normals (`CD_CUSTOMLOOPNORMAL`) and weight-based offsets. One substitution, documented at length: Blender's Eigen least-squares in `adjust_the_cycle_or_chain` becomes normal equations with partial pivoting - the same minimiser, not a different algorithm |
