...
- `research-ai-3d-ecosystem.md` — frontier-AI-era 3D tooling landscape, benchmark evidence on agent API design, and what every other web 3D editor shipped for agents in 2026
...
### 2.4a What the 2026 benchmark evidence says about the agent API

- **The strongest published rationale for this project.** P3D-Bench is the only benchmark to score three.js as a code-generation target. It ties for best validity (1.000, above CadQuery) and matches OpenSCAD on geometry, and loses only on topology, verbatim because "Three.js instead outputs triangulated meshes rather than parametric solids ... such meshes are not guaranteed to be watertight". The deficit is precisely the missing topology-aware representation, which is what this kernel supplies.
- **Feedback types are not interchangeable. Ruler before eyes.** Execution error messages buy syntactic validity almost for free (+27.2 pp executability) and buy no geometric correctness (-0.010). Assertions and measurements buy correctness (+~10 pp, and test-based metrics track human judgement at 0.938 against Chamfer's 0.663). Renders are a weak verifier and a catastrophic *specification* (using renders as the goal spec "collapses every model to near-zero"). So build `measure()` and `validate()` before `renderPreview()`.
- **The dominant editing failure is collateral damage, not the target edit.** BenchCAD reports that ~64% of nominally successful edits silently corrupt unrelated features; BIM-Edit scores geometry 68.9 against topology 37.8. This makes edit locality a first-class measured guarantee (new principle P12): after every transaction, report which elements and attribute layers changed, split into what the operator claimed to touch and what it did not. It also strengthens the per-corner attribute decision: if UVs, creases, seams or material indices silently drop, the agent cannot tell, and the evidence says that is the majority failure case.
- **Keep the tool surface small (new principle P13).** Do not generate 83 MCP tools from the operator table. A model choosing between 131 tools outperforms one choosing from 404; OpenSCAD's small declarative surface beat CadQuery's large fluent one, losing on invalid programs rather than reasoning. Expose a sandboxed `execute` against a persistent session plus a small tool set (measure/describe/selectionInfo/diff/validate/render), and disclose operators progressively.
- **Element references: named handles, not ID soup and not a selector-string DSL.** Revises the earlier Onshape-query recommendation. Keep stable ids for UI and undo, but have scripts register named handles and query with ordinary functions returning plain records. Add a perturbation check that re-runs after nudging a parameter and reports whether a selection resolved to the same elements, which is an automated topological-naming test.
- **Validity is a three-valued contract**: PASS / FAIL / UNVERIFIED with evidence tiers, gated before any other scoring.
- **Node-safety is the differentiator, not a nicety.** Two of the four top BlenderMCP failure modes are that the host cannot be driven headlessly; Blender's own MCP had to grow CLI twins, and the GPT-6 Astra Blender demos are `blender --background --python` plus a render, not GUI control. threepipe starts headless.
- **Correction to an assumption in the brief**: there is no public evidence that kokraf was built with Fable. 510 commits over 16 months by one author, no agent co-author trailers in the last 200 commits, no mention in the README. Worth asking the author directly rather than treating it as fact.
- Every serious web 3D editor shipped an agent surface between June and September 2026 (Babylon, PlayCanvas, Spline, Needle, Onshape); the three.js editor and threepipe have none. No funded, open, general-purpose browser polygon modeller exists.
