...
function catmullClarkStep(positions, cells) {
    ...
    for (let iCell = 0; iCell < cells.length; iCell++) {
        ...
        const facePoints = [];
        const face = { points: [], edges: [], facePoint: [0, 0, 0] };
        faces[iCell] = face;
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        if (boundaryEdges.length >= 2) {
            ...
            const np = [0, 0, 0];
            for (const f of point.faces)
                vec3Add(np, np, f.facePoint);
            for (const e of point.edges)
                mad(np, np, e.midPoint, 2);
 ...
 */
export function catmullClark(positions, cells, levels, toTriangles = true) {
    if (levels < 1)
 ...
 */
export function subdivisionSurface(geometry, level) {
    if (level < 1)
        return geometry;
    ...
    const Ctor = geometry.constructor;
    const newGeom = new Ctor();
    ...
    const AttrCtor = geometry.getAttribute('position').constructor;
