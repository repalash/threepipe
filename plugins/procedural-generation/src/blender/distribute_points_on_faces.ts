...
function float_as_uint(f: number): number {
...
function hash_float_float3(x: number, y: number, z: number): number {
...
function hash_float_to_float_float3(x: number, y: number, z: number): number {
...
class RandomNumberGenerator {
    ...
    get_barycentric_coordinates(): [number, number, number] {
    ...
    // }
    round_probabilistic(x: number): number {
...
function area_tri_v3(v0: Vector3, v1: Vector3, v2: Vector3): number {
...
function normal_tri_v3(v0: Vector3, v1: Vector3, v2: Vector3, outNormal: Vector3): void {
    _edge1.subVectors(v1, v0)
    _edge2.subVectors(v2, v0)
...
// ─── Get triangle vertices from BufferGeometry ─────────────────────
// Helper to read triangle vertex positions from a BufferGeometry.
// In Blender, the mesh has corner_verts and corner_tris.
// In three.js BufferGeometry, triangles are defined by the index buffer
// (or sequential triplets if no index).

interface TriangleData {
    /** Total number of triangles */
    triCount: number
    /** Get the 3 vertex positions for triangle tri_i */
    getTriVerts(tri_i: number, v0: Vector3, v1: Vector3, v2: Vector3): void
    /** Get the 3 loop/corner indices for triangle tri_i (for density factor lookup) */
    getTriCornerIndices(tri_i: number): [number, number, number]
}

function getTriangleData(geometry: BufferGeometry): TriangleData {
    const posAttr = geometry.getAttribute('position') as BufferAttribute
    if (!posAttr) throw new Error('distributePointsOnFaces: geometry has no position attribute')
...
//
// For each triangle:
//   1. Compute density factor as average of vertex density factors
//   2. Compute area
//   3. Seed per-tri RNG with noise::hash(tri_i, seed)
//   4. point_amount = rng.round_probabilistic(area * base_density * factor)
//   5. For each point: generate barycentric coords, interpolate position
...
function sample_mesh_surface(
    triData: TriangleData,
    base_density: number,
    density_factors: Float32Array | null,
    seed: number,
): SampleResult {
    const positions: Vector3[] = []
    const baryCoords: [number, number, number][] = []
    ...
    for (let tri_i = 0; tri_i < triData.triCount; tri_i++) {
        ...
        // source line 142-143
        const point_amount = corner_tri_rng.round_probabilistic(
            area * base_density * corner_tri_density_factor,
        )
...
// ─── KDTree for Poisson elimination ────────────────────────────────
// source/blender/blenlib/BLI_kdtree.hh
//
// Blender uses a 3D KDTree for the Poisson elimination step.
// We implement a simple 3D KDTree that supports:
//   - insert(index, position)
//   - balance()
//   - range_search(position, radius, callback)

interface KDNode {
    index: number
    co: [number, number, number]
    left: number  // index into nodes array, -1 = none
    right: number // index into nodes array, -1 = none
    axis: number  // 0=x, 1=y, 2=z
}

class KDTree3D {
    private nodes: KDNode[] = []
    ...
    insert(index: number, co: Vector3): void {
        this.nodes.push({
            index,
            co: [co.x, co.y, co.z],
            left: -1,
    ...
    private _balance(indices: number[], depth: number): number {
     ...
     */
    range_search(co: Vector3, radius: number, callback: (index: number, dist_sq: number) => boolean): void {
        if (this.root === -1) return
        const radius_sq = radius * radius
        this._range_search(this.root, [co.x, co.y, co.z], radius_sq, callback)
    }

    private _range_search(
        nodeIdx: number,
        co: [number, number, number],
        radius_sq: number,
        callback: (index: number, dist_sq: number) => boolean,
    ): boolean {
        ...
        if (dist_sq <= radius_sq) {
            if (!callback(node.index, dist_sq)) return false
        }
        ...
        if (!this._range_search(first, co, radius_sq, callback)) return false

        // Only search the other side if the splitting plane is closer than radius
        if (diff_sq <= radius_sq) {
            if (!this._range_search(second, co, radius_sq, callback)) return false
        }
...
function update_elimination_mask_for_close_points(
    positions: Vector3[],
    minimum_distance: number,
    elimination_mask: boolean[],
): void {
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        kdtree.range_search(positions[i], minimum_distance, (index: number, _dist_sq: number) => {
            if (index !== i) {
...
function update_elimination_mask_based_on_density_factors(
    triData: TriangleData,
    density_factors: Float32Array,
    baryCoords: [number, number, number][],
    triIndices: number[],
    elimination_mask: boolean[],
): void {
...
function eliminate_points_based_on_mask(
    elimination_mask: boolean[],
    positions: Vector3[],
    baryCoords: [number, number, number][],
    triIndices: number[],
...
export interface DistributePointsOnFacesOptions {
    /** Distribution method. RANDOM = area-weighted random, POISSON = with minimum distance elimination. */
    method: 'RANDOM' | 'POISSON'
    /**
     * For RANDOM mode: per-face density (points per unit area).
     * For POISSON mode: maximum density used for initial oversampling (Density Max).
     */
    density: number
    /**
     * Per-vertex density factor multiplier (0-1). Optional.
     * For RANDOM mode: multiplied per-face (averaged across tri vertices).
     * For POISSON mode: used for post-elimination density-based rejection.
     * If provided, must be a Float32Array with one value per vertex (or per corner/loop).
     */
    densityFactor?: Float32Array
    /** Minimum distance between points (POISSON mode only). */
    minDistance?: number
    /** Random seed. */
    seed: number
}

export interface DistributePointsOnFacesResult {
    /** Generated point positions */
    positions: Vector3[]
    /** Surface normals at each point (interpolated from vertex normals or flat face normals) */
    normals: Vector3[]
    /** Deterministic point IDs (from Blender's hash-based ID computation) */
    ids: number[]
    /** Barycentric coordinates of each point within its triangle (for UV interpolation etc.) */
    baryCoords: [number, number, number][]
    /** Triangle index for each point (index into the triangulated face list) */
    triIndices: number[]
}
...
export function distributePointsOnFaces(
    geometry: BufferGeometry,
    options: DistributePointsOnFacesOptions,
    ...
    let baryCoords: [number, number, number][]
    ...
    if (options.method === 'RANDOM') {
        ...
        const result = sample_mesh_surface(
            triData,
            options.density,
            options.densityFactor ?? null,
            seed,
        )
        ...
        const elimination_mask = new Array<boolean>(positions.length).fill(false)
        update_elimination_mask_for_close_points(
            positions,
