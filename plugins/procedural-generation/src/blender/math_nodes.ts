 ...
 */
export function mapRange(value: number, fromMin: number, fromMax: number, toMin: number, toMax: number, clamp = true): number {
...
export function mapRangeStepped(value: number, fromMin: number, fromMax: number, toMin: number, toMax: number, steps: number): number {
...
export function mapRangeSmoothstep(value: number, fromMin: number, fromMax: number, toMin: number, toMax: number): number {
    if (fromMax === fromMin) return toMin
    let t = (value - fromMin) / (fromMax - fromMin)
...
export function mixFloat(factor: number, a: number, b: number): number {
...
export function compare(a: number, b: number, operation: string): boolean {
    switch (operation) {
...
export function compareFloat(a: number, b: number, operation: string, epsilon = 0.0001): boolean {
    switch (operation) {
    case 'LESS_THAN': return a < b
    case 'LESS_EQUAL': return a <= b
    case 'GREATER_THAN': return a > b
...
export function booleanMath(a: boolean, b: boolean, operation: string): boolean {
    switch (operation) {
    case 'AND': return a && b
    case 'OR': return a || b
    case 'NOT': return !a
...
export function mathOp(operation: string, a: number, b = 0, c = 0): number {
    switch (operation) {
    case 'ADD': return a + b
    ...
    case 'WRAP': { const range = b - c; return range !== 0 ? a - range * Math.floor((a - c) / range) : c }
