...
export function getParamsForRing(id: number, iterations: number): PetalParams {
    const P = PARAM_RANGES
    const lr = (r: {min: number, max: number}) => mapRange(id, 0, iterations, r.min, r.max)
    return {
...
function vectorRotate(vx: number, vy: number, vz: number,
    cx: number, cy: number, cz: number, angle: number, axis: 'X' | 'Y' | 'Z'): [number, number, number] {
    const dx = vx - cx, dy = vy - cy, dz = vz - cz
    const c = Math.cos(angle), s = Math.sin(angle)
    let rx: number, ry: number, rz: number
    if (axis === 'X') { rx = dx; ry = dy * c - dz * s; rz = dy * s + dz * c }
    else if (axis === 'Y') { rx = dx * c + dz * s; ry = dy; rz = -dx * s + dz * c }
    else { rx = dx * c - dy * s; ry = dx * s + dy * c; rz = dz }
    return [rx + cx, ry + cy, rz + cz]
}

// ─── EulerXYZ rotation matrix ───────────────────────────────────────

function eulerXYZMatrix(rx: number, ry: number, rz: number): number[] {
...
function applyRotScale(pos: Float64Array, N: number, rx: number, ry: number, rz: number, sx: number, sy: number, sz: number): void {
...
export function createPetalVertices(p: PetalParams, verticesX = 14, verticesY = 11): Float64Array {
    ...
    if (Math.abs(ww) > 1e-10) {
        const invWW = 1.0 / ww
        for (let i = 0; i < N; i++) pos[i * 3] *= invWW
    }
    ...
    for (let i = 0; i < N; i++) {
        ...
        pos[i * 3] = x + rx; pos[i * 3 + 1] = y + ry; pos[i * 3 + 2] = z + rz
    }
