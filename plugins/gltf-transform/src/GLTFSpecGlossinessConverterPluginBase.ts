...
import {Extension, PlatformIO} from '@gltf-transform/core'
import {ALL_EXTENSIONS} from '@gltf-transform/extensions'
import {GLTFDracoExportPlugin} from './GLTFDracoExportPlugin'
import {metalRough} from '@gltf-transform/functions'

/**
 * GLTFSpecGlossinessConverterPluginBase
 *
 * Base plugin that adds a gltf loader extension that automatically converts GLTF files with specular glossiness materials
 ...
 * to metallic roughness during import.
 *
 * Takes an IO factory function to create PlatformIO instances, allowing use with both WebIO (browser) and NodeIO (Node.js).
 */
export abstract class GLTFSpecGlossinessConverterPluginBase extends AViewerPluginSync {
    enabled = true
    ...
    protected abstract _createIO(): PlatformIO
    ...
    private _loaderCreate({loader}: {loader: GLTFLoader2}) {
        ...
        loader.register(gltfKhrPbrSpecularGlossinessConverterBase(() => this._createIO(), async(m)=>!this.enabled ? false : !this.confirm ? true : this._viewer?.dialog.confirm(this.confirmMessage || m) ?? true, ep?.gltfTransformExtensions))
    }
...
export const gltfKhrPbrSpecularGlossinessConverterBase = (ioFactory: () => PlatformIO, confirm?: (s: string)=>Promise<boolean>, extensions?: (typeof Extension)[])=>(parser: GLTFParser)=>({
    ...
    beforeRoot: async() => {
        try {
            ...
            const io = ioFactory().registerExtensions(ALL_EXTENSIONS).registerExtensions(extensions ?? [])
