...
/**
 * GLTF Draco Exporter Base
 *
 * Extension of GLTFExporter2 that runs the output through gltf-transform for draco compression.
 * Takes a PlatformIO instance as a constructor parameter, allowing use with both WebIO (browser) and NodeIO (Node.js).
 */
export class GLTFDracoExporterBase extends GLTFExporter2 implements IExportWriter {
    public loader?: DRACOLoader2 // required for loading draco libs.
    protected _io: PlatformIO
    private _loadedLibs = false
    private _encoderOptions: EncoderOptions

    constructor(io: PlatformIO, encoderOptions?: EncoderOptions, loader?: DRACOLoader2) {
        super()
        encoderOptions = encoderOptions || {
            method: KHRDracoMeshCompression.EncoderMethod.EDGEBREAKER,
            encodeSpeed: 5,
        }
        this._io = io.registerExtensions(ALL_EXTENSIONS)
