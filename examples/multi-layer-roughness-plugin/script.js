import { _testFinish, _testStart, GeometryGeneratorPlugin, LoadingScreenPlugin, Mesh, MultiLayerRoughnessPlugin, PhysicalMaterial, PickingPlugin, SphereGeometry, ThreeViewer, } from 'threepipe';
...
async function init() {
    ...
    const addSphere = async (name, col, row, roughness, mlr) => {
        const material = new PhysicalMaterial({ name, color: '#ffffff', metalness: 1, roughness });
        if (mlr)
            MultiLayerRoughnessPlugin.AddMultiLayerRoughness(material, mlr);
    ...
    function addLabel(text, x, y, size = 0.13) {
    ...
    const colX = (col) => (col - 1.5) * 1.35;
    const rowY = (row) => (1.5 - row) * 1.35;
    const underLabel = (text, col, row) => addLabel(text, colX(col), rowY(row) - 0.75);
    const rowLabel = (text, row) => addLabel(text, -3.3, rowY(row), 0.15);
